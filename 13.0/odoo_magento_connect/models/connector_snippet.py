...
class ConnectorSnippet(models.TransientModel):
    ...
    def manual_connector_order_operation(self, opr, ecomm, sale_order, opr_obj=False):
        if opr == 'shipment':
            itemData = {}
            # order_lines = sale_order.mapped('order_line').filtered(lambda obj: obj.product_id.prod_type == 'bundle')
            # if order_lines:
            #     itemData = dict(order_lines.mapped(
            #         lambda r: (r.product_id.default_code, int(r.product_uom_qty))))
