...
class StockMove(models.Model):
    ...
    def _action_confirm(self, *args, **kwargs):
        ...
        """
        mobStockAction = "qoh"
        connector_instance_id = self.env['connector.instance'].search([('active', '=', True)], limit=1)
        if connector_instance_id:
            mobStockAction = connector_instance_id.connector_stock_action
    ...
    def fetch_stock_warehouse(self):
        ctx = dict(self._context or {})
        productQuantity = 0
        connectionObj = self.env['connector.instance'].search([('active', '=', True)], limit=1)
        if 'stock_from' not in ctx and connectionObj:
            for data in self:
                ...
                flag = 1
                if data.origin:
                    saleObjs = data.env['sale.order'].search([('name', '=', data.origin)])
                    if saleObjs:
                        get_channel = saleObjs[0].ecommerce_channel
                        if get_channel == 'magento' and data.picking_id  and data.picking_id.picking_type_code == 'outgoing':
                            flag = 0
                else:
                    flag = 2  # no origin
                warehouseId = 0
                ...
                if flag == 2:
                    ...
                    warehouseId = self.check_warehouse_location(locationObj, companyId)  # Receiving Goods
                    if not warehouseId :
                        locationObj = data.location_id
                        warehouseId = self.check_warehouse_location(locationObj, companyId)  # Sending Goods
                data.check_warehouse(odooProductId, sku, warehouseId, productQuantity)
        return True
    ...
    def check_warehouse_location(self, locationObj, companyId):
        ...
        while locationObj :
            warehouseObj = warehouseModel.search([('lot_stock_id', '=', locationObj.id), ('company_id', '=', companyId) ], limit=1)
            if warehouseObj :
    ...
    def check_warehouse(self, odooProductId, sku, warehouseId, productQuantity):
        ctx = dict(self._context or {})
        mappingObj = self.env['connector.product.mapping'].search([('odoo_id', '=', odooProductId)], limit=1)
        if mappingObj:
            ...
            if mappingObj.instance_id.warehouse_id.id == warehouseId:
                ...
                productObj = self.env['product.product'].with_context(ctx).browse(odooProductId)
                ...
                self.synch_quantity(mageProductId, productQuantity, sku, stockItemId, instanceObj)
        return True

    @api.model
    def synch_quantity(self, mageProductId, productQuantity, sku, stockItemId, instanceObj):
        response = self.update_quantity(mageProductId, productQuantity, sku, stockItemId, instanceObj)
        if response[0] == 1:
            return True
        else:
            self.env['connector.sync.history'].create({'status': 'no', 'action_on': 'product', 'action': 'c', 'error_message': response[1]})
            return False
        ...
        if mageProductId:
            if not instanceObj.active:
                ...
                try:
                    ...
                    stockData = {"stockItem": {"itemId": stockItemId,
                                        "productId": mageProductId,
                                        "stockId": 1,
                                        "qty": productQuantity,
                                        "isInStock": True if productQuantity > 0 else False}}
                    self.env['connector.snippet'].callMagentoApi(url='/V1/products/' + str(sku) + '/stockItems/' + str(stockItemId),
                        method='put',
                        data=stockData)
                    return [1, '']
                except Exception as e:
