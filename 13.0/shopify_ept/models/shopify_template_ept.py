...
class ShopifyProductTemplateEpt(models.Model):
    ...
    active_template = fields.Boolean('Active', related="product_tmpl_id.active")
    ...
    total_variants_in_shopify = fields.Integer("Total Varaints", default=0) #modify label name by Nilesh Parmar 20 dec 2019
                ...
                if vals.get('active'):
                    shopify_variants = self.env['shopify.product.product.ept'].search(
                            [('shopify_template_id', '=', shopify_template.id), ('shopify_instance_id', '=',
                                                                         shopify_template.shopify_instance_id.id), ('active', '=', False)])
    ...
    def create_simple_odoo_product_template(self,instance,response_template,product_category,price):
        ...
        barcode=response_template.get('variants')[0].get('sku')
        ...
        if sku or barcode:
            vals={'name': template_title,
                  ...
                  'type': 'product',
                  'categ_id': product_category.id} #  'categ_id': product_category.id
            ...
            available_odoo_products.update({response_template.get('variants')[0]["id"]: product_template.product_variant_id})
    ...
    def shopify_sync_products(self, product_data_line_id, shopify_tmpl_id, instance, log_book_id,
        ...
        for variant in response_template.get('variants'):
            ...
            if not shopify_product:
                if not shopify_template:
                    if not odoo_template and instance.auto_import_product:
                        ...
                        if response_template.get('options')[0].get('name') =='Title' and response_template.get('options')[0].get('values') == ['Default Title'] and len(response_template.get('variants'))==1 :
                            odoo_template, available_odoo_products=self.create_simple_odoo_product_template(instance,response_template,product_category,variant.get('price'))
                        else:
                            if not set_sku_barcode:
                                odoo_template, available_odoo_products = shopify_product_obj.shopify_create_variant_product(response_template,instance,variant.get('price'),product_category,product_data_line_id,False)
                    ...
                    shopify_template = self.create_or_update_shopify_template(instance,
                                                                              ...
                                                                              variant_info,
                                                                              response_template,
                                                                              odoo_product,product_category=product_category,
                ...
                variant_info.update(
                    {"product_id": odoo_product.id, "shopify_template_id": shopify_template.id,"name":odoo_product.name})
            ...
            if instance.sync_product_with_images:
                ...
                if len(shopify_template.shopify_image_ids):
                    image = shopify_template.shopify_image_ids.mapped('image')[0]
                    shopify_template.product_tmpl_id.write({'image_1920':image})
    ...
    # def shopify_prepare_variant_dic(self, variant):
    ...
    def shopify_search_odoo_product_variant(self, shopify_instance, product_sku, variant_id,barcode):
        """
