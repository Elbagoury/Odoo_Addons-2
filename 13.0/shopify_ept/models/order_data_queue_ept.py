...
class ShopifyOrderDataQueueEpt(models.Model):
    ...
    def import_order_cron_action(self, ctx={}):
        ...
        self.shopify_create_order_data_queues(instance, from_date, to_date)

    def shopify_create_order_data_queues(self, instance, from_date, to_date):
        ...
        # without_convert_to_date = to_date
        order_queues = False
        # This is used for convert the time zone of shopify store
        ...
        #         '%Y-%m-%d %H:%M:%S'), "%Y-%m-%d %H:%M:%S") - timedelta(
        #     days=2)  # Change by bhavesh jadav 23/12/2019 for set date two days earlier
        #
