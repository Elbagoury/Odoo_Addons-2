...
class ShopifyProcessImportExport(models.TransientModel):
    ...
    @api.onchange('shopify_product_ids')
    def count_export_product(self):
        self.product_count = len(self.shopify_product_ids)
        ...
        if len(customer_ids) > 0:
            ...
            if len(customer_ids) > 150:
                for customer_id_chunk in split_every(150, customer_ids):
                    customer_queue = self.shopify_create_multi_queue(vals, customer_id_chunk)
                    customer_queue_list.append(customer_queue.id)
            else:
                customer_queue = self.shopify_create_multi_queue(vals, customer_ids)
                customer_queue_list.append(customer_queue.id)
        return customer_queue_list

    def shopify_create_multi_queue(self, vals, customer_ids):
        """Create customer queue and queue line as [er the requirement.
        @author: Angel Patel @Emipro Technologies Pvt. Ltd on date 23/10/2019.
        :Task ID: 157065
        :param vals:
        :param customer_ids:
        ...
        # shopify_customers_data_obj = synced_shopify_customers_data_obj.create(vals)
        ...
        while result:
            ...
            for page_link in link.split(','):
                if page_link.find('next') > 0:
                    ...
                    try:
                        result = shopify.Customer().search(page_info=page_info, limit=250)
